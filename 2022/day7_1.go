...
func main() {
	...
	//iterate through all directories and calculate the size
	runningTotal := tree(root, 4, 0)
...
func tree(curDir Directory, indentSize int, runningTotal int) int {
	indent := ""
	...
	for i := range curDir.dirs {
		runningTotal = tree(curDir.dirs[i], indentSize+4, runningTotal)
	}
	return runningTotal
}
